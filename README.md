...
```shell
  docker buildx build -t discord/musicbot:latest . --load
```

To install the extension, use `make install-extension` **or**:

```shell
  docker extension install discord/musicbot:latest
```
...
```shell
  docker extension dev ui-source discord/musicbot:latest http://localhost:3000
```

In order to open the Chrome Dev Tools for your extension when you click on the extension tab, run:

```shell
  docker extension dev debug discord/musicbot:latest
```

Each subsequent click on the extension tab will also open Chrome Dev Tools. To stop this behaviour, run:

```shell
  docker extension dev reset discord/musicbot:latest
```
...
```shell
docker extension update discord/musicbot:latest
```
...
```shell
docker extension rm discord/musicbot:latest
```
